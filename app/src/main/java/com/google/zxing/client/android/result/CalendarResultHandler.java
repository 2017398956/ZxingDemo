...
public final class CalendarResultHandler extends ResultHandler {
  ...
  public void handleButtonPress(int index) {
    if (index == 0) {
      ...
      addCalendarEvent(calendarResult.getSummary(),
                       System.currentTimeMillis() , // calendarResult.getStartTimestamp(),
                       calendarResult.isStartAllDay(),
                       System.currentTimeMillis() , // calendarResult.getEndTimestamp(),
  ...
  public CharSequence getDisplayContents() {
    ...
    long start = System.currentTimeMillis() ; // calResult.getStartTimestamp();
    ParsedResult.maybeAppend(format(calResult.isStartAllDay(), start), result);

    long end = System.currentTimeMillis() ; // calResult.getEndTimestamp();
    if (end >= 0L) {
